...
const ShareButtons: React.FC<ShareButtonsProps> = ({ content }) => {
  const [copyStatus, setCopyStatus] = useState<string | null>(null);

  const handleCopy = (platform: string) => {
    navigator.clipboard.writeText(content).then(() => {
      setCopyStatus(`Copié pour ${platform} !`);
  ...
  const buttonClass = "flex items-center gap-2 bg-gunmetal/30 text-shadow-blue hover:bg-gunmetal hover:text-platinum px-3 py-2 rounded-lg transition-all duration-300 transform hover:scale-105";

  return (
    <div className="mt-4 pt-4 border-t border-white/10 flex flex-col items-center gap-4">
        <div className="flex items-center gap-4">
            <button onClick={() => handleCopy('X')} className={buttonClass} title="Copier pour X (Twitter)">
                <XIcon />
                <span>X</span>
            </button>
            <button onClick={() => handleCopy('Instagram')} className={buttonClass} title="Copier pour Instagram">
                <InstagramIcon />
                <span>Instagram</span>
            </button>
            <button onClick={() => handleCopy('YouTube')} className={buttonClass} title="Copier pour YouTube">
                <YouTubeIcon />
                <span>YouTube</span>
            </button>
        </div>
        {copyStatus && (
            <p className="text-xs text-cyan-400 animate-fadeIn" role="status">
                {copyStatus}
            </p>
        )}
    </div>
